...
module.exports = {
    ...
    formatDataString(formatString, data, member) {
        const replacements = {
            ...
            "%SMARTNAME%": `${
        data.robloxDisplayName && data.robloxDisplayName !== data.robloxUsername
    ...
    getBindingText(binding, addCodeBlock = false) {
        ...
        if (binding.groups == null) {
            return `\n잘못된 그룹 형식 - 역할 바인딩 해제 ${binding.role} to fix this problem.\n`
        }
        ...
        for (const [index, group] of binding.groups.entries()) {
            ...
            if (group.id.match(/[a-z]/i)) {
                bindingMessage += `# 가상 그룹 ${group.id}\n`
                bindingMessage += `인증 ${
          group.ranks.length > 0 ? group.ranks[0] : "none"
        }`
            } else {
                bindingMessage += `# 그룹 ${group.id}\n`
                bindingMessage +=
                    `계급${group.ranks.length === 1 ? "" : "s"} ` +
